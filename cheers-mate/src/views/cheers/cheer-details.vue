  ...
  <section class="cheer-details-container">
    ...
    <section class="title-attendance-container">
      <div class="name-date dark" :style="backgroundImg">
        ...
        <br>
        <br>
        <span v-if="cheer.attendees" class="attens-num">{{cheer.attendees.length}} mates comming!</span>
      </div>
      ...
      <div class="attending-share">
        <div class="spot-attendance">
          ...
          <div class="is-going" v-else>
            <h4 class="txt">
              ...
              <span
                class="spots-left"
                v-if="cheer.attendees"
              >Spots left: {{spotsLeft}}</span>
            </h4>
            <el-button
              class="btn"
              size="small"
        ...
        <div class="categories">
          ...
          <h2 v-if="cheer.category" v-for="category in cheer.category" :key="category">
            {{category}}
            <br>
          </h2>
    ...
    userAttending(isAttending) {
      ...
      if (currUser) {
        ...
        if (isAttending) {
          ...
          if (idx >= 0) {
            cheerService.removeAttendance(cheerId).then(this.loadCheer);
          }
        }
      } else {
  ...
  computed: {
    ...
    mapPic() {
      return `https://maps.googleapis.com/maps/api/staticmap?center=${
        this.cheer.position.coordinates.lat
      },${this.cheer.position.coordinates.lng}&markers=color:red%7Clabel:C%7C${
        this.cheer.position.coordinates.lat
      },${
        this.cheer.position.coordinates.lng
      }&zoom=16&size=600x400&key=AIzaSyDSpb5jrUSIDb124D7Qpjd4XJQ6d8oVPW0`;
    },
    msgs() {
    ...
    backgroundImg() {
      if (!this.cheer.img)
        return `background-image: url(https://moneycrashers-sparkchargemedia.netdna-ssl.com/wp-content/uploads/2017/08/bachelor-party-bar-drinks-1024x576.jpg)`;
